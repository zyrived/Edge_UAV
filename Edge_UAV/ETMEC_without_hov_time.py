...
class Edge_Uav:
    ...
    def algorithm_2_Update_coordinate(self, offspring, iteration):
        pop_list = Update_corrdinate(offspring, iteration)
        return pop_list
    ...
    def optimize(self):
        ...
        while self.FEs < self.FEs_max:
            ...
            # pop_list = self.algorithm_2_Update_coordinate(offspring, self.FEs)
            # 添加扰动
            ...
            # 检查是否需要计算方差和提前结束循环
            if self.FEs % 1000 == 0:  # 每二十次循环
                if self.FEs >= 500:  # 至少需要十次次循环才能计算方差
