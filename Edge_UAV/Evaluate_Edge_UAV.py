...
class Evaluate:
    ...
    def calculate_time_hov(self, j):
        # 计算第 j 个 UAV 悬停过程中的能耗
        # 实现你的逻辑
        # 返回能耗值
        T_hov = 0
        T_fly = self.calculate_time_fly(j)
    ...
    def total_energy_consumption(self):
        E_UAV = sum((self.calculate_time_fly(j) * self.P_fly + self.calculate_time_hov(j) * self.p_hov) for j in range(len(self.UAv))) + len(self.UAv) * self.E_M
