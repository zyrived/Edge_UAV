...
def tabu_search_algorithm(C, MaxIT, POP):
    ...
    for j in range(len(C)):
        ...
        Path.append(Best['best_sol'])

    data = [list(item) if isinstance(item, np.ndarray) else item for item in Path]
    best_path = [list(set(sublist)) if isinstance(sublist, list) else sublist for sublist in data]
    return best_path
